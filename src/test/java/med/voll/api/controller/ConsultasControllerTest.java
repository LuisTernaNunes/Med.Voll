...
import med.voll.api.domain.consultas.AgendaConsulta;
...
class ConsultasControllerTest {
    ...
    @Autowired
    private JacksonTester<DadosAgendamentoConsulta> DadosConsultaJson;
    ...
    @Test
    @DisplayName("Retorno HTTP 200, Informaçoes Validas")
    @WithMockUser
    void agendar_cenario2() throws Exception{

        var data = LocalDateTime.now().plusHours(4);
        var especialidade = Especialidade.CARDIOLOGIA;
        var response = mvc
                .perform(post("/consultas").contentType(MediaType.APPLICATION_JSON)
                        .content(DadosConsultaJson.write(new DadosAgendamentoConsulta(1L,3L,data,especialidade)).getJson()))
                .andReturn().getResponse();

        assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());
        var jsonEsperado = dadosDetalhamentoConsultasJson.write(new DadosDetalhamentoConsultas(3L,1L,data)).getJson();
